...
if (isset($headers['Authorization'])) {
    ...
    if (preg_match('/Bearer (.+)/', $authorizationHeader, $matches)) {
        ...
        where to_crp_id=" . $user->id . " and is_approved is not null and date between '" . $fromDate . "' and '" . $toDate . "' and product_id=products.id AND type in ('Stock Transfer Note')
        ),0) as stock_in,
        COALESCE(
        (SELECT SUM(quantity) FROM stock_document_details 
        inner join stock_document on stock_document.id=stock_document_details.stock_document_id
        where crp_id=" .  $user->id . " and date between '" . $fromDate . "' and '" . $toDate . "' and product_id=products.id AND type in ('Sales Order')),0)
        stock_out,
        COALESCE((
        SELECT SUM(quantity) FROM stock_document_details 
        inner join stock_document on stock_document.id=stock_document_details.stock_document_id
        ...
        while ($row = pg_fetch_assoc($result)) {
            ...
            $products["data"][] = array(
                ...
                "threshold" =>  (float)$row["crpstock"],
                "availableQuantity" =>  (float)$row["available_quantity"],
                "opening" =>  (float)$row["available_quantity"],
